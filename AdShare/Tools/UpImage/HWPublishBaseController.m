...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return _imageArray.count < _maxCount ?_imageArray.count + 1:_maxCount;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _imageArray.count) {
        ...
        else if (_imageArray.count == _maxCount) {
...
{
    return UIEdgeInsetsMake(6, 10, 6, 0);//分别为上、左、下、右
}
...
- (void)deletePhoto:(UIButton *)sender{
    NSLog(@"%lu****%lu******%ld",(unsigned long)_imageArray.count,(unsigned long)_arrSelected.count,(long)sender.tag);
    [self.imageArray removeObjectAtIndex:sender.tag];
    [self.arrSelected removeObjectAtIndex:sender.tag];
    NSLog(@"%lu****%lu******%ld",(unsigned long)_imageArray.count,(unsigned long)_arrSelected.count,(long)sender.tag);
