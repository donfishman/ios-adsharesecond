...
@property (strong, nonatomic) NSMutableArray *dataArray;//控制器数组

@property (strong, nonatomic) NSString *dataSoureOneStr;// 2数据数组

@property (strong, nonatomic) NSMutableArray *dataSourceTwoArray;// 2数据数组
@property (strong, nonatomic) NSMutableArray *dataSourceThreeArray;// 3数据数组
...
- (void)customPageView {
    ...
    ctrl1.HtmlStr = self.dataSoureOneStr;
    
    PersonalMiddleViewController *ctrl2 = [[PersonalMiddleViewController alloc] init];
    ctrl2.delegate = self;
    ctrl2.dataSoureM =[[NSMutableArray alloc]initWithArray:self.dataSourceTwoArray];

    PersonalRightViewController *ctrl3 = [[PersonalRightViewController alloc] init];
    ctrl3.delegate = self;
    ctrl3.dataSoureR = [[NSMutableArray alloc]initWithArray:self.dataSourceThreeArray];
...
//创建pageViewController  最外层所有的数据
- (void)setPageView:(NSMutableDictionary*)dataDict{
    self.dataSourceTwoArray = [[NSMutableArray alloc]init];
    self.dataSourceThreeArray = [[NSMutableArray alloc]init];
    self.dataSoureOneStr = dataDict[@"msg"][@"content"];
    NSArray * taskArr = dataDict[@"task"];
    for (NSDictionary * dict in taskArr) {
        ...
        [self.dataSourceTwoArray addObject:model];
    }
    NSArray * nearbyArr = dataDict[@"nearby"];
    for (NSDictionary * dictb in nearbyArr) {
        TaskModel * modelb = [TaskModel new];
        modelb.idA = dictb[@"id"];
        modelb.name = dictb[@"name"];
        modelb.thumbnail = dictb[@"thumbnail"];
        modelb.min_price = dictb[@"min_price"];
        modelb.max_price = dictb[@"max_price"];
        modelb.label = dictb[@"label"];
        [self.dataSourceThreeArray addObject:modelb];
    }
    [self customPageView];
...
-(void)tableViewDidSelectId:(NSString *)idStr{
    NSLog(@"+++%@",idStr);
    if (_delegate) {
        [_delegate tableViewDidSelectIdA:idStr];
    }
}
@end
